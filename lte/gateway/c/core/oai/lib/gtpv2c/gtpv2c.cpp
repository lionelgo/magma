...
Gtpv2cStack::Gtpv2cStack(
  ...
  OAILOG_INFO(
      LOG_GTPV2C, "Gtpv2cStack created listening to %s:%d initial seq num %d",
      ip_address.c_str(), port_num, seq_num);

  id              = 0;
  restart_counter = 0;
  udp_s.StartReceive(this, sched_params);
  udp_s_allocated.StartReceive(this, sched_params);
}
//------------------------------------------------------------------------------
uint32_t Gtpv2cStack::GetNextSeqNum() {
...
//------------------------------------------------------------------------------
bool Gtpv2cStack::CheckInitialMessageType(const uint8_t initial) {
  switch (initial) {
...
//------------------------------------------------------------------------------
bool Gtpv2cStack::CheckTriggeredMessageType(
    const uint8_t initial, const uint8_t triggered) {
  OAILOG_INFO(
      LOG_GTPV2C, "check_triggered_message_type GTPV2-C msg type %d/%d",
...
//------------------------------------------------------------------------------
void Gtpv2cStack::StartMsgRetryTimer(
...
#if TRACE_IS_ON
    OAILOG_TRACE(
        LOG_GTPV2C, "Started Msg retry timer %d, proc " PROC_ID_FMT ", seq %d",
...
//------------------------------------------------------------------------------
void Gtpv2cStack::StopMsgRetryTimer(gtpv2c_procedure& p) {
  if (p.retry_timer_id != ITTI_INVALID_TIMER_ID) {
    itti_inst->timer_remove(p.retry_timer_id);
    msg_out_retry_timers.erase(p.retry_timer_id);
#if TRACE_IS_ON
    OAILOG_TRACE(
        LOG_GTPV2C, "Stopped Msg retry timer %d, proc " PROC_ID_FMT ", seq %d",
...
//------------------------------------------------------------------------------
void Gtpv2cStack::StopMsgRetryTimer(timer_id_t& t) {
  itti_inst->timer_remove(t);
  msg_out_retry_timers.erase(t);
#if TRACE_IS_ON
  OAILOG_TRACE(LOG_GTPV2C, "Stopped Msg retry timer %d", t);
#endif
}
//------------------------------------------------------------------------------
void Gtpv2cStack::StartProcCleanupTimer(
    gtpv2c_procedure& p, uint32_t time_out_milli_seconds,
    const task_id_t& task_id, const uint32_t& seq_num) {
...
//------------------------------------------------------------------------------
void Gtpv2cStack::StopProcCleanupTimer(gtpv2c_procedure& p) {
  itti_inst->timer_remove(p.proc_cleanup_timer_id);
#if TRACE_IS_ON
  OAILOG_TRACE(
      LOG_GTPV2C, "Stopped proc cleanup timer %d, proc " PROC_ID_FMT "",
...
void Gtpv2cStack::HandleReceiveMessageCb(
  ...
  if (it == pending_procedures.end()) {
    ...
    if (Gtpv2cStack::CheckInitialMessageType(msg.get_message_type())) {
    ...
    //    OAILOG_INFO(LOG_GTPV2C, "gtpv2c_procedure retry_timer_id        %d",
    //    it->second.retry_timer_id); OAILOG_INFO(LOG_GTPV2C, "gtpv2c_procedure
    //    proc_cleanup_timer_id %d", it->second.proc_cleanup_timer_id);
    //    OAILOG_INFO(LOG_GTPV2C, "gtpv2c_procedure gtpc_tx_id            %ld",
    //    it->second.gtpc_tx_id); OAILOG_INFO(LOG_GTPV2C, "gtpv2c_procedure
    //    initial_msg_type      %d", it->second.initial_msg_type);
    //    OAILOG_INFO(LOG_GTPV2C, "gtpv2c_procedure triggered_msg_type    %d",
    //    it->second.triggered_msg_type); OAILOG_INFO(LOG_GTPV2C,
    ...
    if (Gtpv2cStack::check_triggered_message_type(
...
uint32_t Gtpv2cStack::SendInitialMessage(
  ...
  OAILOG_TRACE(
      LOG_GTPV2C, "Sending %s, seq %d, proc " PROC_ID_FMT " ",
      gtp_ies.get_msg_name(), msg.get_sequence_number(), gtp_tx_id);
  gtpv2c_procedure proc = {};
  proc.initial_msg_type = msg.get_message_type();
...
uint32_t Gtpv2cStack::SendInitialMessage(
  ...
  OAILOG_TRACE(
      LOG_GTPV2C, "Sending %s, seq %d, teid " TEID_FMT ", proc " PROC_ID_FMT "",
  ...
  if (it != msg_out_retry_timers.end()) {
    ...
    if (it_proc != pending_procedures.end()) {
      if (it_proc->second.retry_count < n3) {
        ...
        // send again message
        OAILOG_TRACE(
            LOG_GTPV2C, "Retry %d Sending msg type %d, seq %d",
            it_proc->second.retry_count,
