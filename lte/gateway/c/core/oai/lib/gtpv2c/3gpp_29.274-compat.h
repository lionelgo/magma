...
#ifndef FILE_3GPP_29_274_COMPAT_SEEN
#define FILE_3GPP_29_274_COMPAT_SEEN
extern "C" {
...
#define IMSI_BCD8_SIZE 8
  ...
  gtpc_imsi_s() : num_digits(0) { memset(u1.b, 0, sizeof(u1.b)); }

  gtpc_imsi_s(const gtpc_imsi_s& i) : num_digits(i.num_digits) {
...
typedef struct gtpc_imsi_s gtpc_imsi_t;

//-------------------------------------
// 8.4 Cause
typedef struct gtpc_cause_s {
  uint8_t cause_value;
  uint8_t pce : 1;
  uint8_t bce : 1;
  uint8_t cs : 1;
...
//-------------------------------------
// 8.9 IP Address
typedef struct gtpc_ip_address_s {
...
#define MSISDN_MAX_LENGTH (15)
  ...
  gtpc_msisdn_s() : num_digits(0) { memset(u1.b, 0, sizeof(u1.b)); }
...
typedef struct gtpc_msisdn_s gtpc_msisdn_t;
...
typedef struct gtpc_pdn_type_s {
  uint8_t pdn_type;
  gtpc_pdn_type_s() : pdn_type(PDN_TYPE_E_IPV4) {}
  gtpc_pdn_type_s(const uint8_t& p) : pdn_type(p) {}
  gtpc_pdn_type_s(const struct gtpc_pdn_type_s& p) : pdn_type(p.pdn_type) {}
  bool operator==(const struct gtpc_pdn_type_s& p) const {
...
typedef struct gtpc_paa_s gtpc_paa_t;
//-------------------------------------
...
typedef struct gtpc_bearer_qos_s {
  uint8_t spare1 : 1;
  uint8_t pci : 1;
  uint8_t pl : 4;
  uint8_t spare2 : 1;
  ...
  bool operator==(const struct gtpc_bearer_qos_s& q) const {
    return (
...
typedef struct gtpc_flow_qos_s {
  uint8_t label_qci;
  uint64_t maximum_bit_rate_for_uplink;
  uint64_t maximum_bit_rate_for_downlink;
  uint64_t guaranted_bit_rate_for_uplink;
...
struct gtpc_rat_type_s {
  uint8_t rat_type;
  gtpc_rat_type_s() : rat_type(RAT_TYPE_E_EUTRAN_WB_EUTRAN) {}
  gtpc_rat_type_s(const rat_type_e r) : rat_type(r) {}
  gtpc_rat_type_s(const gtpc_rat_type_s& i) : rat_type(i.rat_type) {}
  //----------------------------------------------------------------------------
  std::string toString() const {
    switch (rat_type) {
...
typedef struct gtpc_rat_type_s gtpc_rat_type_t;
//-------------------------------------
...
struct interface_type_s {
  ...
  interface_type_s(interface_type_t t) : interface_type(t) {}
...
typedef struct interface_type_s gtpc_interface_type_t;
...
struct fully_qualified_tunnel_endpoint_identifier_s {
  ...
  std::string toString() const {
    std::string s = {};
    gtpc_interface_type_t iface_type((interface_type_t) interface_type);
    if ((v4) || (v6)) {
...
typedef struct fully_qualified_tunnel_endpoint_identifier_s gtpc_fteid_t;
...
} gtpc_charging_characteristics_t;
...
// typedef struct fully_qualified_gtpc_cause_s {
//  uint8_t spare1 :4;
//  uint8_t gtpc_cause_type :4;
//  char* f-cause_field;
//} fully_qualified_gtpc_cause_t;
...
enum gtpc_node_id_type_e {
  GTPC_GLOBAL_UNICAST_IPv4 = 0,
  GTPC_GLOBAL_UNICAST_IPv6 = 1,
  GTPC_TYPE_EXOTIC = 2,  ///< (MCC * 1000 + MNC) << 12 + Integer value assigned
                         ///< to MME by operator
};
...
//-------------------------------------
// 8.65 Node Type
enum gtpc_node_type_e { NODE_TYPE_E_MME = 0, NODE_TYPE_E_SGSN = 1 };
...
typedef struct allocation_retention_priority_s {
  ...
  uint8_t spare2 : 1;
  uint8_t pvi : 1;
} gtpc_allocation_retention_priority_t;
typedef gtpc_allocation_retention_priority_t arp_t;
...
enum ran_nas_gtpc_cause_type_e {
...
typedef struct ran_nas_gtpc_cause_s {
...
template<>
class hash<gtpc_fteid_t> {
 public:
  size_t operator()(const gtpc_fteid_t& k) const {
    using std::hash;
    using std::size_t;
...
#endif /* FILE_3GPP_29_274_COMPAT_SEEN */
